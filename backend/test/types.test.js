...
// Each verse should be correctly identified as its expected metre type.
// Run with: node --test backend/test/types.test.js
...
// Check that the detected metre string includes the expected keyword.
async function assertMetre (verse, expected, label) {
  const metre = await getMetre(verse.trim())
  assert.ok(
    metre.includes(expected),
    `${label}: expected metre to include "${expected}", got "${metre}"`
  )
}
...
describe('வெண்பா', () => {
  it('ஒரு விகற்ப குறள் வெண்பா', async () => {
    await assertMetre(
...
குற்றமொன்று இல்லா அறம்`,
  ...
  it('இரு விகற்ப குறள் வெண்பா', async () => {
    await assertMetre(
...
கானக நாடன் சுனை`,
  ...
  it('ஒரு விகற்ப நேரிசை வெண்பா', async () => {
    await assertMetre(
...
போற்றினா னல்கும் பொருள்`,
      'வெண்பா', 'நேரிசை வெண்பா'
    )
  })

  it('இரு விகற்ப நேரிசை வெண்பா', async () => {
    await assertMetre(
...
கண்ணி னானடி யேயடை வார்கள் கற்றவரே`,
  ...
  it('குறள் வெண்செந்துறை', async () => {
    await assertMetre(
      `போதிநிழற் புனிதன் பொலங்கழல்
ஆதி உலகிற் காண்`,
      'குறள்வெண்செந்துறை', 'குறள் வெண்செந்துறை'
    )
  })
...
நீங்கா இன்பமொடு நீடுவாழ் கெனவே`,
      'ஆசிரியப்பா', 'ஆசிரியப்பா'
    )
  })

  it('நிலைமண்டில ஆசிரியப்பா', async () => {
    await assertMetre(
...
மரங்கொல் மணந்தகன்றார் நெஞ்சமென் செய்த திளவேனில்`,
      'ஆசிரியத்துறை', 'ஆசிரியத்துறை'
    )
  })

  it('அறுசீர் ஆசிரிய விருத்தம்', async () => {
    await assertMetre(
...
தொழுகைய இமையவரும் அறம்மருவு துதிசெய்தெழு துடித புரமே!`,
      'ஆசிரியவிருத்தம்', 'ஆசிரியவிருத்தம்'
    )
  })
})
...
என்செய் வேன்கச்சி யேகம்ப நாதனே`,
      'கட்டளைக்கலிப்பா', 'கட்டளைக்கலிப்பா'
    )
  })

  it('கட்டளை கலித்துறை', async () => {
    await assertMetre(
...
describe('வஞ்சிப்பாவினம்', () => {
  it('வஞ்சித்துறை', async () => {
    await assertMetre(
      `பொருந்து போதியில்
இருந்த மாதவர்
திருந்து சேவடி
மருந்து ஆகுமே`,
      'வஞ்சித்துறை', 'வஞ்சித்துறை'
    )
  })

  it('வஞ்சி விருத்தம்', async () => {
    await assertMetre(
      `அணிதங்கு போதி வாமன்
